class Contact {
    ...
    async update() {
        ...
        let response = await $.ajax("http://localhost:3010/contacts/"+this.id,
        {
            type: "POST",
            dataType: "json",
...
Contact.findAll = async () => {
    let response = await $.ajax("http://localhost:3010/contacts/", {type: "GET", dataType: "json"});
...
Contact.find = async (id) => {
    try {
        let response = await $.ajax("http://localhost:3010/contacts/"+id,
        {
            type: "GET",
            dataType: "json"
        });
...
Contact.create = async (first, last, email, phone, notes, url) => {
    console.log("here")
    let data_string = JSON.stringify({
        first: first,
        last: last,
    ...
    let response = await $.ajax("http://localhost:3010/contacts",
    {
        type: "POST",
        dataType: "json",
        contentType: "application/json",
